...
resource "aws_s3_bucket" "cloud-resume-site-bucket" {
  ...
  logging {
    target_bucket = aws_s3_bucket.cloud-resume-logging-bucket.id
  }
...
#tfsec:ignore:aws-s3-encryption-customer-key
resource "aws_s3_bucket" "cloud-resume-logging-bucket" {
  bucket = "tf-aws-mr-nget-1-cloud-resume"
...
resource "aws_s3_bucket_public_access_block" "cloud-resume-logging-bucket" {
  bucket                  = aws_s3_bucket.cloud-resume-logging-bucket.id
  block_public_acls       = true
  block_public_policy     = true
  ignore_public_acls      = true
  restrict_public_buckets = true
